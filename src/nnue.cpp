...
void activate(nnue::accumulator &accumulator, Square sq, Piece p, Square ksq_white,
    ...
    for (int i = 0; i < 512; i += 16) {
        __m256i input =
            _mm256_loadu_si256((__m256i *)&INPUT_WEIGHTS[input_white * N_HIDDEN_SIZE + i]);

        __m256i acc = _mm256_loadu_si256((__m256i *)&accumulator[0][i]);

        __m256i result = _mm256_add_epi16(input, acc);

        _mm256_storeu_si256((__m256i *)&accumulator[0][i], result);
    }

    // Loop over the full accumulator array
    for (int i = 0; i < 512; i += 16) {
        __m256i input =
            _mm256_loadu_si256((__m256i *)&INPUT_WEIGHTS[input_black * N_HIDDEN_SIZE + i]);

        __m256i acc = _mm256_loadu_si256((__m256i *)&accumulator[1][i]);

        __m256i result = _mm256_add_epi16(input, acc);

        _mm256_storeu_si256((__m256i *)&accumulator[1][i], result);
    }
}
...
void move(nnue::accumulator &accumulator, Square from_sq, Square to_sq, Piece p, Square ksq_white,
    ...
    for (int i = 0; i < 512; i += 16) {
        __m256i clearInput =
            _mm256_loadu_si256((__m256i *)&INPUT_WEIGHTS[input_clear_white * N_HIDDEN_SIZE + i]);

        __m256i addInput =
            _mm256_loadu_si256((__m256i *)&INPUT_WEIGHTS[input_add_white * N_HIDDEN_SIZE + i]);
    ...
    for (int i = 0; i < 512; i += 16) {
        __m256i clearInput =
            _mm256_loadu_si256((__m256i *)&INPUT_WEIGHTS[input_clear_black * N_HIDDEN_SIZE + i]);

        __m256i addInput =
            _mm256_loadu_si256((__m256i *)&INPUT_WEIGHTS[input_add_black * N_HIDDEN_SIZE + i]);
...
void init(const char *filename) {
    ...
    // obtain file size
    std::size_t fileSize =
        FEATURE_SIZE * N_HIDDEN_SIZE + N_HIDDEN_SIZE + 2 * N_HIDDEN_SIZE + OUTPUTS;
