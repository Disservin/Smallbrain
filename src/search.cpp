...
Score Search::qsearch(Score alpha, Score beta, Stack *ss) {
    ...
    TEntry *tte = TTable.probe(ttHit, ttMove, board.hash_key);
    Score ttScore =
    ...
    Movelist moves;
    FastLookup<QSEARCH> mp(*this, ss, moves, ttMove);
...
moves:
    ...
    FastLookup<ABSEARCH> mp(*this, ss, moves, searchmoves, RootNode, ttHit ? ttMove : NO_MOVE);
