...
void uciOptions::printOptions() {
    for (const auto &option : optionsPrint) {
        std::cout << "option name " << option.name << " type " << option.type << " default "
                  << option.defaultValue;
        if (option.min != "") std::cout << " min " << option.min << " max " << option.max;
...
int uciOptions::uciThreads(int value) { return std::clamp(value, 1, 512); }
